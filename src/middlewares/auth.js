    ...
    console.log(profile)
        ...
        if (user) {
          jwt.sign({user},'noSecrets', (err,token) =>{
            ...
            const newUser = await models.User.create({email:profile.email,names:profile.displayName, roleId: role.dataValues.id,verified:profile.email_verified});
  ...
  console.log(profile)
      ...
      if (user) {
        jwt.sign({user},'noSecrets', (err,token) =>{
          ...
          const role = await models.Role.findOne({where: {name: 'requester'}})
          const newUser = await models.User.create({email:profile.emails[0].value,names:profile.displayName, roleId: role.dataValues.id,verified:true});
          jwt.sign({newUser},'noSecrets', (err,token) =>{
            if(err){
              return err
            }
